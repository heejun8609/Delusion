...
def job():
    ...
    sta_date = datetime.datetime.strptime('2017-09-22', "%Y-%m-%d")

    def master_acc():
        connection = pymysql.connect(host='cb-cbt-seoul-db-replica.c730awg3qklt.ap-northeast-2.rds.amazonaws.com',
    ...
    def user_game_log():
        connection0 = pymysql.connect(host='cb-cbt-seoul-db-replica.c730awg3qklt.ap-northeast-2.rds.amazonaws.com',
    ...
    def user_match_log_wins():
        connection0 = pymysql.connect(host='cb-cbt-seoul-db-replica.c730awg3qklt.ap-northeast-2.rds.amazonaws.com',
                                     user='cb_user', passwd='cb!23viewer11', db='rts_user0', charset='utf8')
        with connection0.cursor() as cursor0:
            cursor0.execute("SELECT * FROM rts_user0._user_match_log_wins")
            fet0 = cursor0.fetchall()
        connection0.close()
        columns0 = []
        for x in cursor0.description:
    ...
    user_match_log_wins = user_match_log_wins()
    ...
    def cat_le_time(con, league):
        ...
        def cat_le_result(x):
            ...
            y = cat_le_count.pivot_table(index='cur_point_cat', columns='castle_cat', values='user_id').fillna(0).astype(int)
            ...
            y.to_excel('./1. league_castle/1) '+yester+'_catle_league_' + con + '_CBT.xlsx')
    ...
    d1_cat_le = cat_le_time('day1_act', league_cat)
    d1_cat_le_50 = cat_le_time('day1_act', league_dr)

    d1_user_cat = d1_cat_le.groupby('user_id')['cur_point_cat'].all().reset_index()
    le_cat = d1_user_cat.groupby('cur_point_cat')['user_id'].size().reset_index().set_index('cur_point_cat')
    ...
    day1_act_u = pd.DataFrame(day1_act_user.groupby('user_id')['user_id'].all())

    castle_lv = castle_lv.set_index('user_id')

    cas_lv = day1_act_u.join(castle_lv).drop('user_id', axis=1).reset_index().groupby('castle_level')['user_id'].size().reset_index().set_index('castle_level')
    ...
    le_cas_lv.to_excel('./1. league_castle/1) '+yester + '_day_league_castle_base_CBT.xlsx')
    ...
    wait_time_sec = user_match_log_begin[(user_match_log_begin['event_time'] >= yester) &
                                         ...
                                         (user_match_log_begin['match_type'] == 1)].loc[:,['user_id','opponent_id', 'point', 'wait_time_sec']]
    ...
    match_succ = pd.DataFrame(user_match_log_begin[(user_match_log_begin['event_time'] >= str(today-datetime.timedelta(days=2))) &
                                                   (user_match_log_begin['event_time'] < str(today)) &
    ...
    match_success = round((match[match['match'] == 1].groupby('cur_point_cat')['match'].size()/match[match['match'] == 0].groupby('cur_point_cat')['match'].size()).reset_index(), 2)
    ...
    final_cp_diff.to_excel('./2. wait_cp/2) '+yester+'_wait_cpdiff_CBT.xlsx')
    ...
    #         match_df = match_df[(match_df['end_reason'] == 3)]
        ...
        # 1(타운홀 파괴, 항복), 3(접속종료)
    #     if end_reason == 1:
        match_table.to_excel('./3. match_ratio/3) '+yester+'_match_ratio_CBT.xlsx')
    #     elif end_reason == 3:
    ...
    match_end = user_match_log_end.loc[:, ['user_id', 'match_id', 'event_time', 'end_reason', 'win',
                                           ...
                                           'opponent_unlock_card_ref_id_1', 'opponent_unlock_card_ref_id_2', 'opponent_unlock_card_ref_id_3',
                                           'opponent_unlock_card_ref_id_4', 'opponent_unlock_card_ref_id_5', 'opponent_unlock_card_ref_id_6']]
    match_begin = user_match_log_begin.loc[:, ['user_id', 'match_id', 'opponent_id', 'castle_level', 'match_type', 'point',
    ...
    daeyun = ml_temp[(ml_temp['match_type'] == 1) &
                      ...
                      (ml_temp['event_time'] >= str(today-datetime.timedelta(days=1))) &
    ...
    top_5per = user_league.sort_values('cur_point', ascending=False).iloc[:int(len(user_league)/20)].loc[:,['user_id', 'cur_point']]
    cut_line = top_5per['cur_point'].iat[-1]
    wungsub = top_5per.merge(wungsub, on='user_id')
    wungsub.drop('cur_point', axis=1, inplace=True)
    ...
    for card_mat in work_list:
        ...
        card = user_game_log[(user_game_log['param_type'] == 20) &
                          ...
                          (user_game_log['param2'] == 101)].loc[:, ['user_id','event_time']].sort_values('event_time')
        ...
        opp = opp.rename(columns={'opponent_win':'win', 'opponent_hero':'hero', 'opponent_hero_random':'hero_random', 'opponent_unlock_card_ref_id_1':'unlock_card_ref_id_1', 'opponent_unlock_card_ref_id_2':'unlock_card_ref_id_2', 'opponent_unlock_card_ref_id_3':'unlock_card_ref_id_3', 'opponent_unlock_card_ref_id_4':'unlock_card_ref_id_4', 'opponent_unlock_card_ref_id_5':'unlock_card_ref_id_5', 'opponent_unlock_card_ref_id_6':'unlock_card_ref_id_6'})
        ...
        for x in card_list:
            if x.drop(x.columns[-1], axis=1).sum(axis=0).sum() != 0:
                s_dic[x.columns[-1]] = {'pick_count' : x.drop(x.columns[-1], axis=1).sum(axis=0).sum(),
                                       'pick_rate(%)' : round(x.drop(x.columns[-1], axis=1).sum(axis=0).sum() / len(x), 2),
                                       'win_rate(%)' : round(x[x[x.columns[-1]] == 1].ix[:,0:8].sum(axis=0).sum() / x.drop(x.columns[-1], axis=1).sum(axis=0).sum(), 4)}
            else:
        ...
        pick_win = pick_win.merge(card_dic_df, on='name').drop('num', axis=1).set_index('name').sort_values(by='pick_rate(%)', ascending=False)
        writer = pd.ExcelWriter('./4. card_pick_win/4) '+yester+'_card_pic_win_ratio_'+work_name[cc]+'_CBT.xlsx')
        pick_win.to_excel(writer,'카드 픽률&승률')
        pd.DataFrame({'tier1' : tier1, 'tier2' : tier2, 'tier3' : tier3}, index=['tier_count']).to_excel(writer,'Tier Count')
        writer.save()
        cc += 1
    ...
    end_temp = user_match_log_end[(user_match_log_end.event_time >= yester) &
                                    (user_match_log_end.event_time < str(today))].loc[:, ['user_id', 'match_id', 'end_reason']]

    end_reason_ratio = end_temp.groupby(['end_reason', 'user_id'])['match_id'].size().sum(level=0) / end_temp.groupby(['end_reason', 'user_id'])['match_id'].size().sum(level=0).sum()
    ...
    user_end_reason_3 = end_temp.groupby(['end_reason', 'user_id'])['match_id'].size()[3] / end_temp.groupby(['end_reason', 'user_id'])['match_id'].size()[3].sum()
    ...
    outer = MIMEMultipart()
    outer['Subject'] = str(today)+' Daily Statistics(CBT)'
#     outer['To'] = 'vmurmurv@naver.com'
    ...
    filepath_list = ['./1. league_castle/1) '+yester+'_catle_league_all_CBT.xlsx',
                     './1. league_castle/1) '+yester+'_catle_league_day1_act_CBT.xlsx',
                     './1. league_castle/1) '+yester+'_day_league_castle_base_CBT.xlsx',
                     './2. wait_cp/2) '+yester+'_wait_cpdiff_CBT.xlsx',
                     './3. match_ratio/3) '+yester+'_match_ratio_CBT.xlsx',
                     './4. card_pick_win/4) '+yester+'_card_pic_win_ratio_daeyun_CBT.xlsx',
                     './4. card_pick_win/4) '+yester+'_card_pic_win_ratio_wungsub_CBT.xlsx']

    text = '''
    1. 유저 League & Castle Lv 분포(전체, 최근 1일)

    2. 전날 0 ~ 24시 동안의 랭크전 평균 매칭 대기 시간 & 평균 점수 차이

    3. 전날 0 ~ 24시 동안의 매칭 비율

    4. (대윤님)직전 1일 동안의 카드 픽률 & 승률(랭크, User+AI)

    4. (웅섭님)직전 1일 동안의 카드 픽률 & 승률(랭크, User, 상위 5%) -> 게임수: '''+str(int(len(wungsub)))+''', Cut line: '''+str(cut_line)+''' 

    5. End_reason Ratio -> '''+str(end_retension)+''', 접속종료 평균 초과 user_id -> '''+str(over_user_id) +'''
    ...
    * 자세한 내용은 '원노트 기획 -> 박희준 -> 데이터 요청사항' 확인 요망
    ...
    for filename in filepath_list:
        ...
        msg.add_header('Content-Disposition', 'attachment', filename=filename[file_num[0]-3:])
        outer.attach(msg)
...
class Scheduler():
    ...
    def scheduler(self):
        ...
        trigger = CronTrigger(day_of_week='mon-sun', hour='8', minute='40')
